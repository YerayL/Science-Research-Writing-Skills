# Appendix A: Prefixes Used in Science Writing

## Core Idea
A reference list of common prefixes in scientific English, with meanings and examples. Mastering these helps decode unfamiliar terms and construct precise scientific vocabulary.

## Key Prefix Table

| Prefix | Meaning | Examples |
|--------|---------|----------|
| a-, an- | not | asymmetrical, anaerobic |
| ante- | before | antechamber, antenatal |
| anti- | against | antimicrobial, antioxidant |
| auto- | self | autonomous, autocorrect |
| bi- | two | binary, bicarbonate |
| cent- | hundred/hundredth | centigrade, centimeter |
| chrom- | colour | chromatic, chromosome |
| chron- | time | chronological, chronometer |
| circ- | around | circulate, circumnavigate |
| co- | with/together | coauthor, coordinate |
| contra- | against | contraflow, contraindicate |
| counter- | against | counteract, counterpoint |
| crypt- | hidden | cryptocurrency, cryptography |
| de- | apart/away | decompose, dehydrate |
| di- | two | dichloride, dioxide |
| dis- | apart/away, not | dislocate, dissimilar |
| equi- | equal | equivalent, equilateral |
| hetero- | different | heterogeneous |
| homo- | same | homogeneous |
| hyper- | above/over | hyperactive, hypertension |
| hypo- | below/under | hypodermic, hypotension |
| inter- | between | international, interact |
| intra- | within | intracellular, intravenous |
| iso- | equal | isometric, isotope |
| kilo- | thousand | kilogram, kilometer |
| mal- | bad | malfunction, malnutrition |
| mega- | large/million | megabyte, megalith |
| meta- | beyond/change | metamorphic, metadata |
| milli- | thousandth | millimeter, millisecond |
| mis- | wrong | misalign, miscalculate |
| mono- | one | monomer, monoxide |
| multi- | many | multidisciplinary, multiphase |
| nano- | billionth | nanometer, nanoparticle |
| neo- | new | neolithic, neonatal |
| non- | not | non-linear, non-toxic |
| over- | excess | overestimate, overexpose |
| poly- | many | polymer, polycrystalline |
| post- | after | postdoctoral, post-treatment |
| pre- | before | precursor, pretreatment |
| pseudo- | false | pseudoplastic, pseudocode |
| re- | again | reproduce, recalibrate |
| semi- | half | semiconductor, semipermeable |
| sub- | under | substrate, subdivision |
| super- | above/beyond | superimpose, supersaturate |
| trans- | across | transform, translucent |
| tri- | three | triaxial, triglyceride |
| ultra- | beyond | ultrasonic, ultraviolet |
| un- | not | unstable, unreacted |

## Key Takeaway
Understanding these prefixes provides a systematic way to build and decode scientific vocabulary, especially useful for non-native English speakers.

## Connects To
- **Ch 8 (Checklist)**: Consistent vocabulary usage
- **Ch 2 (Methods)**: Prefixes appear frequently in technical method descriptions
